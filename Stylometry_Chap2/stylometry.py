...
def stopwords_test(words_by_author, len_shortest_corpus):
    ...
    for i, author in enumerate (words_by_author):
        stopwords_by_author = [word for word in
            words_by_author[author][:len_shortest_corpus] if word in stop_words
        ]
        ...
        stopwords_by_author_freq_dist[author].plot(
            ...
            , label=author
            , linestyle=LINES[i]
            , title='50 Most Common Stopwords ' + author
        )
    plt.legend()
    #plt.show()
...
def jaccard_test(words_by_author, len_shortest_corpus):
    """Calculate Jaccard similiarity of each known corpus to unknown corpus."""
...
def main():
    ...
    strings_by_author['doyle'] = text_to_string('hound.txt', 'utf-8')
    # strings_by_author['wells'] = text_to_string('war.txt', 'utf-8')
    strings_by_author['wells'] = text_to_string('war.txt', 'utf-8')
    strings_by_author['unknown'] = text_to_string('lost.txt', 'utf-8')
